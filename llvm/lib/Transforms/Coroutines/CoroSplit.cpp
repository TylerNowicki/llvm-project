...
CoroSplitPass::CoroSplitPass(bool OptimizeFrame)
    : GenABI([](Function &F, coro::Shape &S) { return CreateNewABI(F, S); }),
...
CoroSplitPass::CoroSplitPass(std::function<bool(Instruction &)> IsMatCallback,
                             bool OptimizeFrame)
    : GenABI([=](Function &F, coro::Shape &S) {
        return CreateNewABIIsMat(F, S, IsMatCallback);
      }),
      OptimizeFrame(OptimizeFrame) {}
...
PreservedAnalyses CoroSplitPass::run(LazyCallGraph::SCC &C,
  ...
  for (LazyCallGraph::Node *N : Coroutines) {
    ...
    coro::Shape Shape(F);
    if (!Shape.CoroBegin) {
      Shape.invalidateCoroutine(F);
      continue;
    }

    F.setSplittedCoroutine();

    std::unique_ptr<coro::BaseABI> ABI(GenABI(F, Shape));
    ABI->init();
